...
async function submitAnswer(){
  ...
  const userId = el('#userId').value.trim() || `user-${Math.random().toString(36).slice(2,7)}`;
  ...
  await fetch(api('/api/answer'), {
    ...
    body: JSON.stringify({ questionId: state.questionId, userId, ciphertext, iv, salt })
  });
  state.submitted = true; el('#status').textContent = 'Answer locked. Waiting for your partner…';
}
async function revealAnswers(){
  const passphrase = el('#passphrase').value.trim();
  const me = el('#userId').value.trim();
  const { answers } = await fetch(api(`/api/answers/${state.questionId}`)).then(r=>r.json());
  if(answers.length < 2) return;
  let myText = '', partnerText = '';
  for (const a of answers) {
    try { const plain = await decryptText(a.ciphertext, a.iv, a.salt, passphrase);
      if (a.userId === me) myText = plain; else partnerText = plain;
    } catch(e){}
  }
  if(!myText && !partnerText){ el('#status').textContent = 'Could not decrypt. Did you both type the same passphrase?'; return; }
